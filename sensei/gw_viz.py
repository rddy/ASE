# Adapted from https://github.com/HumanCompatibleAI/evaluating-rewards/blob/master/src/evaluating_rewards/analysis/reward_figures/gridworld_reward_heatmap.py
...
def _reward_make_color_map(
    ...
    return matplotlib.cm.ScalarMappable(norm=norm, cmap=matplotlib.cm.binary)
...
def _reward_draw_spline(
    ...
    if reward >= 2:
        colors = ['orange', 'green']
        color = colors[int(reward)-2]
        rgba_of_color = {
          'orange': (1, 0.5, 0, 1),
          'green': (0, 1, 0, 1)
        }
        color = rgba_of_color[color]
    else:
    ...
    ang = action - 1
    ang = (ang + 1) % 4
    state = env.state_from_pos(np.array([x, y, ang]))
    text = str(state)
...
def _reward_draw(
    ...
    while not it.finished:
        ...
        vert, color, hatch_color = _reward_draw_spline(
            env, x, y, action, optimal, reward, from_dest, mappable, annot_padding, ax
        )
        ...
        if hatch:  # draw the hatch using a different color
            ...
            patches.append(fn(vert, tuple(color), linewidth=1, edgecolor=edgecolor))#, fill=fill))
    ...
    **kwargs,
    ...
    if highlight_wall:
        for edge in ['bottom', 'top', 'right', 'left']:
            base_ax.spines[edge].set_color('orange')
            base_ax.spines[edge].set_linewidth(5)
    ...
    for idx, (pretty_name, reward) in enumerate(reward_arrays.items()):
        ...
        _reward_draw(env, reward, 0.99, fig=fig, ax=ax, mappable=mappable, **kwargs)
